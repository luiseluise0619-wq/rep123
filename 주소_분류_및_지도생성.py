# -*- coding: utf-8 -*-
"""
주소 → 분류 → 엑셀(지역별 시트) → 모바일 지도(HTML)  [초보자용 설명]
================================================================================

이 파일이 하는 일 (전체 흐름을 쉽게)
    ┌ 1) 엑셀에서 '주소'와 '사업장 수'를 읽는다.
    │ 2) 주소 글자를 정규표현식으로 쪼개 '시도 / 구군 / 동네'로 분류한다.
    │      예) "서울특별시 강남구 역삼동 …" → 시도=서울특별시, 구군=강남구, 동네=역삼동
    │ 3) 시도별로 시트를 나눠 엑셀 한 파일(지역별_주소_분류.xlsx)로 저장한다.
    │      (분류가 안 되는 이상한 주소는 '기타' 시트로 모음)
    └ 4) 주소를 지도에 점으로 찍는 독립 실행형 HTML(모바일_구글지도.html)을 만든다.
         - 서버 없이 HTML 파일을 더블클릭하면 바로 열림
         - 배경지도는 API 키가 필요 없는 구글 지도 타일 사용

용어
    · '정규표현식(regex)' = "이런 모양의 글자를 찾아줘"라고 컴퓨터에 알려주는 규칙.
    · 'openpyxl' = 파이썬으로 엑셀(.xlsx)을 읽고 쓰는 도구.

입력 엑셀 컬럼 (exceldata1.xlsx 기준)
    A열 : 주소        C열 : 사업장 수(같은 주소에 여러 사업장일 수 있음)

결과 엑셀 컬럼 배치
    A:원본주소  B:시도  C:구군  D:동네  E:상세주소  F:분류상태  G:사업장수

★ 개인정보: 전화번호·사업자등록번호는 공개 결과물(엑셀·지도·웹)에 넣지 않는다.
   (예전엔 넣었지만, 지도를 공개하면 노출되므로 주소·사업장수만 남기도록 바꿈)

주의(윈도우/인코딩)
    · 윈도우 경로의 역슬래시(\\) 에러를 막으려고 경로에 r'' (raw string)을 쓴다.
    · 모든 파일 입출력은 UTF-8 로 처리해 한글이 깨지지 않게 한다.
"""
...
COL_ADDR = 1   # A열 : 주소
COL_CNT  = 3   # C열 : 사업장 수 (한 주소에 여러 사업장일 수 있음)
COL_TEL  = 4   # D열 : 전화번호       (※ 개인정보라 결과엔 안 넣음 · 현재 입력엔 없음)
COL_BIZ  = 5   # E열 : 사업자 등록번호 (※ 개인정보라 결과엔 안 넣음 · 현재 입력엔 없음)
